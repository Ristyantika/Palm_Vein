...
datanama = dir('D:\KULIAH\SEMESTER 8\TA\Coba\DataPalm\*.jpg');
for i=1:length(datanama)    
    tmp = strcat('D:\KULIAH\SEMESTER 8\TA\Coba\DataPalm\',datanama(i).name);
    fileo = strcat('D:\KULIAH\SEMESTER 8\TA\Coba\DataPalm\Preprocessing168\Versi2\', datanama(i).name);
    img_inn = imread(tmp);
...
%img_inn = imread('D:\KULIAH\SEMESTER 8\TA\DataPalm\001_l_940_02.jpg');
    ...
    % ROI - Noise Removal
    se = strel('disk',17);        
    imgbw = imopen(imgbw_in,se);
    ctrImg = zeros(size(imgbw,1),size(imgbw,2));
